...
class CocoClsDataset(data.Dataset):
    def __init__(self, root_dir, ann_file, img_dir, bg_bboxes_file, phase, less_sample=False, eq=False):
        ...
        if phase == 'train' and less_sample:
            self.ann_ids, self.bg_anns = self._mining_sample()

        print('total_length of dataset:{}={}+{}'.format(len(self), len(self.ann_ids), len(self.bg_anns)))
    ...
    def _mining_sample(self):
        ...
        if self.eq:
            num_bg_anns = len(tmp_ann_ids)
            start_idx = 20000
            tmp_bg_anns = self.bg_anns[start_idx:start_idx + num_bg_anns]
        else:
            num_bg_anns = 20000
            tmp_bg_anns = self.bg_anns[::len(self.ann_ids) // num_bg_anns]
        self.num_dict['background'] = len(tmp_bg_anns)
        return tmp_ann_ids, tmp_bg_anns
    ...
    def __getitem__(self, idx):
        ...
        if label != 0:
            label -= 1
        binary_label = torch.zeros(2)
        binary_label[bin_label] = 1
